...
class TFIDFTaxonomyResultsAggregator:
    """
    ...
    def __init__(self, parquet_file_path: str, column_name: str = "tfidf_taxonomy_classification"):
        """
        Initialize the aggregator with data from a parquet file.

        Args:
            parquet_file_path (str): Path to the parquet file containing tfidf results
        """
    ...
    def _prepare_demographics(self):
        ...
        required_cols = ["age", "gender", "industry"]

        missing = [col for col in required_cols if col not in schema]
        if missing:
            print(f"Warning: Missing columns for demographic aggregation: {missing}")
            return  # Skip demographic aggregation if any required columns are missing
    ...
    def _aggregate_category_classifications_for_group(
        ...
        for row in group_df.iter_rows(named=True):
            classifications = self._parse_tfidf_classification_json(row[self.column_name])
